...
import { SMTP_STATUS_OPTIONS } from 'src/_mock/_table/_admin/_users';
...
export function SmtpTable({ onEditClick, handleOpenDialog }) {
  ...
  const handleConfirmDelete = () => {
    setDeleteOpen(false);
    toast.success('Connected SMTP deleted successfully.');
  };
          ...
          <Box display="flex" justifyContent="space-between" flexDirection={{xs:'column',md:'row'}} alignItems={{xs:'flex-end',md:'center'}}>
            <Box display="inline-block" mb={{xs:2,md:0}}>
              ...
              <Typography sx={{ mt: '4px' }} variant="body2" color="grey.600">
                Manage SMTP servers for email notifications. Add, update, enable, or disable
