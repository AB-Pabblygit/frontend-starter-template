...
function ModalVideoView({
  ...
  // Replace your existing imageSrc line with this:
  const imageSrc = `${CONFIG.site.basePath}/assets/images/${
    getHelp === false
      ? `big-card-thumbhnails${isAppRoute ? '/app' : '/admin'}`
      : `get-help-video-thumbnails${isAppRoute ? '/app' : '/admin'}`
  }/${thumbnailName}`;
        ...
        <Tooltip disableInteractive title="Click to watch tutorial." arrow placement="top">
          <div>
            ...
            >
              <img
                width={50}
                src={`${CONFIG.site.basePath}/assets/icons/play-icons/play-icon.svg`}
                alt="sdsd"
              />
            </IconButton>
          </div>
